...
#define NVENC_IDLE_TIMEOUT_DEFAULT	10000	/* 10 milliseconds */
#define NVENC_IDLE_CHECK_PERIOD		10	/* 10 usec */

#define get_nvenc(ndev) ((struct nvenc *)(ndev)->dev.platform_data)
#define set_nvenc(ndev, f) ((ndev)->dev.platform_data = f)

/* caller is responsible for freeing */
static char *nvenc_get_fw_name(struct platform_device *dev)
{
	char *fw_name;
	u8 maj, min;
	...
	decode_nvenc_ver(pdata->version, &maj, &min);
	sprintf(fw_name, "nvhost_nvenc0%d%d.fw", maj, min);
	dev_info(&dev->dev, "fw name:%s\n", fw_name);

	return fw_name;
}

static int nvenc_dma_wait_idle(struct platform_device *dev, u32 *timeout)
{
	if (!*timeout)
		*timeout = NVENC_IDLE_TIMEOUT_DEFAULT;

	do {
		u32 check = min_t(u32, NVENC_IDLE_CHECK_PERIOD, *timeout);
		u32 dmatrfcmd = nvhost_device_readl(dev, nvenc_dmatrfcmd_r());
		u32 idle_v = nvenc_dmatrfcmd_idle_v(dmatrfcmd);

		if (nvenc_dmatrfcmd_idle_true_v() == idle_v)
			return 0;

		udelay(NVENC_IDLE_CHECK_PERIOD);
		*timeout -= check;
	} while (*timeout);
...
static int nvenc_dma_pa_to_internal_256b(struct platform_device *dev,
		u32 offset, u32 internal_offset, bool imem)
{
	u32 cmd = nvenc_dmatrfcmd_size_256b_f();
	u32 pa_offset =  nvenc_dmatrffboffs_offs_f(offset);
	u32 i_offset = nvenc_dmatrfmoffs_offs_f(internal_offset);
	u32 timeout = 0; /* default*/

	if (imem)
		cmd |= nvenc_dmatrfcmd_imem_true_f();

	nvhost_device_writel(dev, nvenc_dmatrfmoffs_r(), i_offset);
	nvhost_device_writel(dev, nvenc_dmatrffboffs_r(), pa_offset);
	nvhost_device_writel(dev, nvenc_dmatrfcmd_r(), cmd);

	return nvenc_dma_wait_idle(dev, &timeout);

}

static int nvenc_wait_idle(struct platform_device *dev, u32 *timeout)
{
	if (!*timeout)
		*timeout = NVENC_IDLE_TIMEOUT_DEFAULT;
	...
	do {
		u32 check = min_t(u32, NVENC_IDLE_CHECK_PERIOD, *timeout);
		u32 w = nvhost_device_readl(dev, nvenc_idlestate_r());
...
int nvenc_boot(struct platform_device *dev)
{
	u32 timeout;
	u32 offset;
	int err = 0;
	struct nvenc *m = get_nvenc(dev);

	/* check if firmware is loaded or not */
	if (!m || !m->valid)
		return -ENOMEDIUM;

	nvhost_device_writel(dev, nvenc_dmactl_r(), 0);
	nvhost_device_writel(dev, nvenc_dmatrfbase_r(),
		(sg_dma_address(m->pa->sgl) + m->os.bin_data_offset) >> 8);

	for (offset = 0; offset < m->os.data_size; offset += 256)
		nvenc_dma_pa_to_internal_256b(dev,
					   m->os.data_offset + offset,
					   offset, false);

	nvenc_dma_pa_to_internal_256b(dev, m->os.code_offset, 0, true);

	/* setup nvenc interrupts and enable interface */
	nvhost_device_writel(dev, nvenc_irqmset_r(),
			(nvenc_irqmset_ext_f(0xff) |
				nvenc_irqmset_swgen1_set_f() |
				nvenc_irqmset_swgen0_set_f() |
				nvenc_irqmset_exterr_set_f() |
				nvenc_irqmset_halt_set_f()   |
				nvenc_irqmset_wdtmr_set_f()));
	nvhost_device_writel(dev, nvenc_irqdest_r(),
			(nvenc_irqdest_host_ext_f(0xff) |
				nvenc_irqdest_host_swgen1_host_f() |
				nvenc_irqdest_host_swgen0_host_f() |
				nvenc_irqdest_host_exterr_host_f() |
				nvenc_irqdest_host_halt_host_f()));
	nvhost_device_writel(dev, nvenc_itfen_r(),
			(nvenc_itfen_mthden_enable_f() |
				nvenc_itfen_ctxen_enable_f()));

	/* boot nvenc */
	nvhost_device_writel(dev, nvenc_bootvec_r(), nvenc_bootvec_vec_f(0));
	nvhost_device_writel(dev, nvenc_cpuctl_r(),
			nvenc_cpuctl_startcpu_true_f());

	timeout = 0; /* default */

	err = nvenc_wait_idle(dev, &timeout);
	if (err != 0) {
...
static int nvenc_setup_ucode_image(struct platform_device *dev,
		u32 *ucode_ptr,
		const struct firmware *ucode_fw)
{
	struct nvenc *m = get_nvenc(dev);
	/* image data is little endian. */
	struct nvenc_ucode_v1 ucode;
	int w;
	...
	ucode.bin_header = (struct nvenc_ucode_bin_header_v1 *)ucode_ptr;
	...
	ucode.os_header = (struct nvenc_ucode_os_header_v1 *)
...
int nvenc_read_ucode(struct platform_device *dev, const char *fw_name)
{
	struct nvenc *m = get_nvenc(dev);
	const struct firmware *ucode_fw;
	int err;
	...
	if (!ucode_fw) {
		dev_err(&dev->dev, "failed to get nvenc firmware\n");
		err = -ENOENT;
		return err;
	}
	...
	err = nvenc_setup_ucode_image(dev, (u32 *)m->mapped, ucode_fw);
	if (err) {
...
int nvhost_nvenc_init(struct platform_device *dev)
{
	...
	fw_name = nvenc_get_fw_name(dev);
	if (!fw_name) {
		dev_err(&dev->dev, "couldn't determine firmware name");
		return -EINVAL;
	}

	m = kzalloc(sizeof(struct nvenc), GFP_KERNEL);
	if (!m) {
	...
	err = nvenc_read_ucode(dev, fw_name);
	kfree(fw_name);
	fw_name = 0;
...
{
	return nvenc_boot(dev);
}

static struct of_device_id tegra_nvenc_of_match[] = {
#ifdef TEGRA_21X_OR_HIGHER_CONFIG
	{ .compatible = "nvidia,tegra210-nvenc",
		.data = (struct nvhost_device_data *)&t21_nvenc_info },
#endif
	{ },
};

#ifdef CONFIG_PM_GENERIC_DOMAINS
static int nvenc_unpowergate(struct generic_pm_domain *domain)
{
...
static int nvenc_powergate(struct generic_pm_domain *domain)
{
	struct nvhost_device_data *pdata;
...
static int nvenc_probe(struct platform_device *dev)
{
	int err = 0;
	...
	if (dev->dev.of_node) {
		...
		match = of_match_device(tegra_nvenc_of_match, &dev->dev);
		if (match)
	...
	pdata->pdev = dev;
	pdata->init = nvhost_nvenc_init;
	pdata->deinit = nvhost_nvenc_deinit;
	pdata->finalize_poweron = nvhost_nvenc_finalize_poweron;
...
#ifdef CONFIG_PM_GENERIC_DOMAINS
	pdata->pd.name = "nvenc";
	pdata->pd.power_off = nvenc_powergate;
	pdata->pd.power_on = nvenc_unpowergate;
...
static int __exit nvenc_remove(struct platform_device *dev)
{
#ifdef CONFIG_PM_RUNTIME
	pm_runtime_put(&dev->dev);
...
static struct platform_driver nvenc_driver = {
	.probe = nvenc_probe,
	.remove = __exit_p(nvenc_remove),
	.driver = {
		.owner = THIS_MODULE,
		.name = "nvenc",
#ifdef CONFIG_OF
...
static int __init nvenc_init(void)
{
	return platform_driver_register(&nvenc_driver);
}

static void __exit nvenc_exit(void)
{
	platform_driver_unregister(&nvenc_driver);
}
