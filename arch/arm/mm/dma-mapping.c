			 ...
			 struct dma_attrs *attrs)
{
	...
	for (i = 0; i < count; ) {
		...
		len = (j - i) << PAGE_SHIFT;
		ret = pg_iommu_map(mapping, iova, phys, len, (int)attrs);
		if (ret < 0)
			goto fail;
		iova += len;
		i = j;
	...
	return dma_addr;
fail:
	pg_iommu_unmap(mapping, dma_addr, iova-dma_addr, (int)attrs);
...
skip_cmaint:
	...
	*handle = iova_base;
	...
	return 0;
fail:
	pg_iommu_unmap(mapping, iova_base, count * PAGE_SIZE, (int)attrs);
